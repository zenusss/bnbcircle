...
import { ChevronRight, ChevronLeft, CheckCircle, MapPin, Upload, X, Shield } from "lucide-react";
...
export default function AdminPropertyEditor() {
  ...
  const onSubmit = (data: any) => {
    console.log("Admin submit", data, selectedAmenities, photos);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      {/* Admin badge */}
      <div className="flex items-center gap-2 px-3 py-2 bg-accent/10 border border-accent/20 rounded-xl w-fit">
        <Shield className="w-4 h-4 text-accent" />
        <span className="text-sm font-semibold text-accent">Admin Mode</span>
      </div>

      <div>
        <h1 className="text-2xl font-bold text-primary">
          {isEditing ? "Admin: Edit Listing" : "Admin: Add Listing"}
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          {isEditing
            ? "Editing listing on behalf of a host. Changes take effect immediately."
            : "Creating a new listing. It will be published directly without host review queue."}
        </p>
      </div>
      ...
      <div className="card-base p-6">
        ...
        {step === 0 && (
          <div className="space-y-5">
            ...
            <div>
              ...
              <textarea {...register("description", { required: true })} rows={5} className="input-base resize-none" placeholder="Describe the property, its highlights, and nearby attractions..." />
            </div>
            <div>
        ...
        {step === 1 && (
          <div className="space-y-5">
            <h2 className="text-lg font-bold text-primary">Where is this property?</h2>
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <label className="border-2 border-dashed border-border rounded-2xl p-12 text-center hover:border-accent transition-colors cursor-pointer flex flex-col items-center">
        ...
        {step === 3 && (
          <div className="space-y-5">
            <h2 className="text-lg font-bold text-primary">What amenities are offered?</h2>
        ...
        {step === 4 && (
          <div className="space-y-5">
            <h2 className="text-lg font-bold text-primary">Set pricing</h2>
            <div className="grid grid-cols-2 gap-4">
              <div>
            ...
            <div className="p-4 bg-accent/5 border border-accent/20 rounded-xl text-sm">
              <span className="flex items-center gap-2 text-accent font-semibold mb-1">
                <Shield className="w-4 h-4" /> Admin note
              </span>
              <p className="text-muted-foreground">Listings created by admins bypass the host review queue and are published immediately.</p>
            </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-5">
            ...
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-emerald-800 text-sm">
              ✓ As admin, this listing will be published immediately without review.
            </div>
            <div className="space-y-3 text-sm">
              {[
                { label: "Title", value: watch("title") || "—" },
                { label: "Property type", value: watch("property_type") || "—" },
                { label: "Location", value: `${watch("city") || "—"}, ${watch("region") || "Netherlands"}` },
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={step === 0 ? () => navigate("/admin/listings") : goBack}
          className="btn-outline flex items-center gap-2"
        >
          <ChevronLeft className="w-4 h-4" />
          {step === 0 ? "Back to listings" : "Back"}
        </button>
        {step < STEPS.length - 1 ? (
